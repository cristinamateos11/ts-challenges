...
let resultGen2 = getArray2<number>([1, 2, 3, 4, 5]);
...
function persona<T, U>(name: T, age: U): string {
  let msg = `Hola, ${name} ! Tienes ${age} años.`;
  return msg;
}

let result3 = persona<string, number>("Cristina", 23);
console.log(result3);

let result4 = persona<boolean, number>(true, 22);
console.log(result4);
 ...
 */
function helado<T extends validTypes, U>(name: T, price: U): string {
  let msg = "";
...
let comanda = helado<string, number>("chocolate", 2.5);
let comanda2 = helado<number, number>(2, 2.8);
...
let processResult = new processIdentity<number, string>(100, "hello world");
